...
$$
S_t=S_0 \prod_{n=0}^{t-1}(1+g_n)
$$

where $$t$$ is the time \(in years\) elapsed since the token sale, $$S_t$$ is the total token supply at time $$t$$,$$S_0$$ is the number of tokens minted at time of token sale and $$g_t:=\frac{S_{t+1}-S_t}{S_t}$$ is the growth rate of $$S_t$$ between times $$t$$ and $$t+1$$.
